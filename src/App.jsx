...
const DARK = {
  ...
  textFaint: "#444",
  accent:    "#c9a96e",
  headerBg:  "linear-gradient(180deg,#1a1a24 0%,transparent 100%)",
  statsBg:   "#111118",
  inputBg:   "#111118",
  editBg:    "#111827",
  presentOff:"#15151c",
...
const LIGHT = {
  ...
  textFaint: "#aaa",
  accent:    "#a0782a",
  headerBg:  "linear-gradient(180deg,#eceae4 0%,transparent 100%)",
  statsBg:   "#ede9e2",
  inputBg:   "#ffffff",
  editBg:    "#fdf8f0",
  presentOff:"#f0ede8",
...
const TI = ({ value, onChange, placeholder, type="text", min, max, t }) =>
  <input style={{
    background: t.inputBg, border: "1px solid "+t.border3, borderRadius: 7,
    color: t.text, padding: "7px 10px", fontSize: 13, fontFamily: "inherit",
...
const Badge = ({ color, children }) =>
  <span style={{
    ...
    borderRadius: 4, padding: "1px 6px", fontSize: 10, fontWeight: 700, letterSpacing: .3,
  }}>{children}</span>;
  ...
  return (
    <div style={{ padding:"12px 20px",
      ...
      <span style={{ fontFamily:"'Playfair Display',serif", fontWeight:700,
        color:t.text, fontSize:15, letterSpacing:.4 }}>{label}</span>
...
function EditRow({ s, onSave, onCancel, t }) {
  ...
  return (
    <div style={{ background:t.editBg, border:"1px solid "+t.accent+"55",
      borderRadius:10, padding:"14px 16px", display:"flex", flexDirection:"column", gap:12 }}>
      <div style={{ fontSize:11, color:t.accent, fontWeight:700, letterSpacing:.5, textTransform:"uppercase" }}>
        ✎ Bearbeiten
      </div>
...
function ThemeToggle({ dark, onToggle, t }) {
  return (
    <button onClick={onToggle} style={{
      ...
      fontFamily:"inherit", fontSize:12, fontWeight:600, color:t.textSub,
      transition:"all .2s",
    }}>
...
export default function App() {
  ...
  const toggleToday = room => {
    ...
    if (inToday) setToday(p => p.filter(r => r.id !== room.id));
  ...
  const addFloorToToday = floor => {
    const newRooms = roomDB.filter(r => r.floor===floor && !today.some(t => t.id===r.id));
    setToday(p => [...p, ...newRooms.map(r => ({ ...r, type:"abreise" }))]);
  };

  return (
    <div style={{ minHeight:"100vh", background:t.bg, color:t.text,
      fontFamily:"'DM Sans','Segoe UI',sans-serif", fontSize:14, transition:"background .3s,color .3s" }}>
      <style>{`
        ...
        *{box-sizing:border-box;}
        ...
        option { background: ${t.inputBg}; color: ${t.text}; }
      `}</style>

      {/* Header */}
      <div style={{ borderBottom:"1px solid "+t.border, padding:"16px 30px",
        display:"flex", alignItems:"center", justifyContent:"space-between",
        background: t.headerBg }}>
        <div>
          <div style={{ fontFamily:"'Playfair Display',serif", fontSize:22, fontWeight:900,
            color:t.accent, letterSpacing:.5 }}>Zimmerservice Planer</div>
          <div style={{ fontSize:11, color:t.textMuted, marginTop:2 }}>
            Faire Personalplanung · Housekeeping · {dateStr}
          </div>
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:20 }}>
          <ThemeToggle dark={darkMode} onToggle={() => setDarkMode(d => !d)} t={t} />
      ...
      <div style={{ padding:"22px 28px", maxWidth:1200, margin:"0 auto" }}>
          ...
          <div>
            {today.length===0 && (
              <div style={{ textAlign:"center", padding:60, color:t.textMuted }}>
                ...
                <span style={{ color:t.accent }}>→ Tab „Tagesplan" → Zimmer aktivieren</span>
              </div>
            )}
            {today.length>0 && activeStaff.length===0 && (
              <div style={{ textAlign:"center", padding:60, color:t.textMuted }}>
                Keine Mitarbeiter anwesend.<br/>
                <span style={{ color:t.accent }}>→ Tab „Personal" → Anwesenheit aktivieren</span>
              </div>
            )}
            {today.length>0 && activeStaff.length>0 && (
              <>
                <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(290px,1fr))", gap:16 }}>
                  {activeStaff.map(s => {
                    ...
                    return (
                      <div key={s.id} style={{ background:t.bgCard, border:"1px solid "+t.border,
                        borderRadius:14, overflow:"hidden" }}>
                        <div style={{ padding:"14px 18px",
                          background:"linear-gradient(90deg,"+t.accent+"12 0%,transparent 100%)",
                          borderBottom:"1px solid "+t.border }}>
                          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                            <div>
                              <div style={{ fontFamily:"'Playfair Display',serif",
                                fontSize:16, fontWeight:700, color:t.text }}>{s.name}</div>
                        ...
                        <div style={{ padding:"12px 16px", display:"flex", flexDirection:"column", gap:6 }}>
                          {sRooms.length===0 && <span style={{ color:t.textFaint, fontSize:12 }}>Keine Zimmer zugewiesen</span>}
                          {sRooms.map(r => (
                            <div key={r.id} style={{ display:"flex", alignItems:"center",
                              justifyContent:"space-between", background:t.bgInner,
                              border:"1px solid "+t.border2, borderRadius:8, padding:"6px 11px" }}>
                              <div style={{ display:"flex", alignItems:"center", gap:7 }}>
                                <span style={{ fontFamily:"'Playfair Display',serif",
                                  fontSize:14, fontWeight:700, color:t.accent }}>{r.number}</span>
                ...
                <div style={{ marginTop:20, background:t.bgCard, border:"1px solid "+t.border,
        ...
        {tab==="tagesplan" && (
          <div>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16, flexWrap:"wrap", gap:8 }}>
              <div style={{ fontSize:13, color:t.textSub }}>{today.length} Zimmer für heute ausgewählt</div>
              <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                {FLOORS.map(f => (
                  <Btn key={f} v="ghost" sm t={t} onClick={() => addFloorToToday(f)}>+ Etage {f}</Btn>
                ))}
            ...
            {FLOORS.map(floor => {
              ...
              return (
                <Card key={floor} t={t}>
                  <CardHead
                    label={floor+". Etage"} color={FLOOR_C[floor-1]} t={t}
                  ...
                  <div style={{ padding:14, display:"flex", flexWrap:"wrap", gap:8 }}>
                    {floorRooms.map(r => {
                      const todayEntry = today.find(t2 => t2.id===r.id);
                      const active = !!todayEntry;
                      return (
                        <div key={r.id} style={{ display:"flex", flexDirection:"column", gap:5,
                          background: active ? FLOOR_C[floor-1]+"18" : t.bgInner,
                          border:"1px solid "+(active ? FLOOR_C[floor-1]+"55" : t.border2),
                          borderRadius:10, padding:"10px 12px", transition:"all .2s", minWidth:110 }}>
                          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:6 }}>
                            <button onClick={() => toggleToday(r)} style={{
                              background:"none", border:"none", cursor:"pointer",
                              fontFamily:"'Playfair Display',serif", fontSize:16, fontWeight:700,
                              color: active ? FLOOR_C[floor-1] : t.textFaint,
                          ...
                          {active && (
                            <div style={{ display:"flex", gap:4 }}>
                              {["bleibe","abreise"].map(tp => (
                                <button key={tp} onClick={() => setRoomType(r.id, tp)} style={{
                                  flex:1,
                                  background: todayEntry.type===tp ? TYPE_C[tp]+"33" : "transparent",
                                  border:"1px solid "+(todayEntry.type===tp ? TYPE_C[tp] : t.border2),
                                  borderRadius:5, color: todayEntry.type===tp ? TYPE_C[tp] : t.textMuted,
                                  fontSize:10, fontWeight:700, cursor:"pointer",
                                  padding:"3px 0", fontFamily:"inherit",
                                }}>{tp==="bleibe"?"Bleibe":"Abreise"}</button>
                              ))}
                            </div>
                          )}
                        </div>
        ...
        {tab==="personal" && (
          <div>
            {/* Anwesenheit */}
            <Card t={t}>
              <CardHead label="Anwesenheit heute" color="#3a9e6a" t={t}
                sub={activeStaff.length+" von "+staff.length+" anwesend"} />
              <div style={{ padding:"14px 18px", display:"flex", flexWrap:"wrap", gap:10 }}>
                {staff.length===0 && <span style={{ color:t.textFaint, fontSize:12 }}>Noch kein Personal angelegt.</span>}
                {staff.map(s => {
                  const on = !!present[s.id];
                  return (
                    <button key={s.id} onClick={() => togglePresent(s.id)} style={{
                      display:"flex", alignItems:"center", gap:10,
                      background: on ? "#3a9e6a14" : t.presentOff,
                      border:"1px solid "+(on ? "#3a9e6a55" : t.border2),
                      borderRadius:10, padding:"10px 14px",
                      cursor:"pointer", transition:"all .2s", fontFamily:"inherit",
                    }}>
                      <div style={{ width:34, height:34, borderRadius:"50%", flexShrink:0,
                        background: on ? "#3a9e6a28" : t.avatarOff,
                        border:"2px solid "+(on ? "#3a9e6a" : t.border),
                        display:"flex", alignItems:"center", justifyContent:"center",
                        fontSize:13, fontWeight:700, color: on ? "#3a9e6a" : t.textMuted,
                      }}>{s.name[0]}</div>
                      <div style={{ textAlign:"left" }}>
                        <div style={{ fontWeight:700, fontSize:13, color: on ? t.text : t.textMuted }}>{s.name}</div>
                        <div style={{ fontSize:10, color: on ? t.textSub : t.textFaint, marginTop:1 }}>
                          {s.hours}h · {s.age} J.
                        </div>
                      </div>
                      <div style={{ width:18, height:18, borderRadius:5, flexShrink:0,
                        background: on ? "#3a9e6a" : "transparent",
                        border:"2px solid "+(on ? "#3a9e6a" : t.border),
                        display:"flex", alignItems:"center", justifyContent:"center",
            ...
            <Card t={t}>
              ...
              <div style={{ padding:"14px 18px",
                ...
                <Field label="Name" t={t}>
                  <TI value={nStaff.name} onChange={v=>setNStaff(p=>({...p,name:v}))} placeholder="Vorname Nachname" t={t} />
                </Field>
                <Field label="Alter" t={t}>
                  <TI type="number" min={16} max={70} value={nStaff.age} onChange={v=>setNStaff(p=>({...p,age:v}))} t={t} />
                </Field>
                <Field label="Stunden / Tag" t={t}>
                  <TI type="number" min={1} max={12} value={nStaff.hours} onChange={v=>setNStaff(p=>({...p,hours:v}))} t={t} />
                </Field>
              ...
              <div style={{ padding:16, display:"flex", flexDirection:"column", gap:8 }}>
                ...
                {staff.map(s => (
                  editId===s.id
                    ...
                    : (
                      <div key={s.id} style={{ background:t.bgInner, border:"1px solid "+t.border2,
                        borderRadius:10, padding:"11px 15px",
                        display:"flex", alignItems:"center", justifyContent:"space-between", gap:10, flexWrap:"wrap" }}>
                        <div style={{ display:"flex", gap:11, alignItems:"center" }}>
                          <div style={{ width:36, height:36, borderRadius:"50%", flexShrink:0,
                            background: present[s.id] ? "#3a9e6a20" : t.avatarOff,
                            border:"1px solid "+(present[s.id]?"#3a9e6a55":t.border),
                            display:"flex", alignItems:"center", justifyContent:"center",
                            fontSize:14, fontWeight:700, color: present[s.id]?"#3a9e6a":t.textMuted,
                          }}>{s.name[0]}</div>
                          <div>
                            <div style={{ fontWeight:700, color:t.text, fontSize:14 }}>{s.name}</div>
                            <div style={{ fontSize:11, color:t.textSub, marginTop:2 }}>
                              {s.age} Jahre · {s.hours}h/Tag ·
                              Faktor <span style={{ color:t.accent }}>×{AGE_FACTOR(s.age).toFixed(2)}</span> ·
                              Kapazität <span style={{ color:TYPE_C.bleibe }}>{staffCap(s).toFixed(1)} Pkt</span>
                            </div>
                          </div>
                        </div>
        ...
        {tab==="db" && (
          <div>
            <Card t={t}>
              <CardHead label="Zimmer hinzufügen" color={t.accent} t={t} />
              <div style={{ padding:"14px 18px",
                display:"grid", gridTemplateColumns:"1fr 1fr 1fr auto", gap:12, alignItems:"flex-end" }}>
                <Field label="Etage" t={t}>
                  <Sel value={nRoom.floor} onChange={v=>setNRoom(p=>({...p,floor:+v}))} t={t}
                    options={FLOORS.map(f=>({ value:f, label:f+". Etage" }))} />
                </Field>
                <Field label="Zimmernummer" t={t}>
                  <TI value={nRoom.number} onChange={v=>setNRoom(p=>({...p,number:v}))} placeholder="z.B. 307" t={t} />
                </Field>
                <Field label="Größe" t={t}>
                  <Sel value={nRoom.size} onChange={v=>setNRoom(p=>({...p,size:v}))} t={t}
                    options={[{value:"S",label:"Klein (S)"},{value:"M",label:"Mittel (M)"},{value:"L",label:"Groß (L)"}]} />
                </Field>
                <Btn onClick={addRoomDB} t={t}>+ Hinzufügen</Btn>
              </div>
            </Card>
              ...
              return (
                <Card key={floor} t={t}>
                  ...
                  <div style={{ padding:14, display:"flex", flexWrap:"wrap", gap:8 }}>
                    ...
                    {fRooms.map(r => (
                      <div key={r.id} style={{ background:t.bgInner, border:"1px solid "+t.border2,
                        borderRadius:9, padding:"8px 12px", display:"flex", alignItems:"center", gap:8 }}>
                        <span style={{ fontFamily:"'Playfair Display',serif",
                          fontSize:15, fontWeight:700, color:t.accent }}>{r.number}</span>
                        <Badge color={SIZE_C[r.size]}>{SIZE_L[r.size]}</Badge>
                        <Btn v="red" sm t={t} onClick={()=>removeRoomDB(r.id)}>×</Btn>
                      </div>
                    ))}
                  </div>
                </Card>
